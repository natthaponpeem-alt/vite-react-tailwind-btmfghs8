...
export default function App() {
  ...
  if (!dbInitialized) {
    return (
      <div className="min-h-screen bg-[#f7f7f7] flex flex-col items-center justify-center space-y-4">
  ...
  return (
    <div className="min-h-screen bg-[#f7f7f7] text-stone-900 pb-24" style={{ fontFamily: "'Inter', 'Noto Sans Thai', system-ui, sans-serif" }}>
      <style>{`
      ...
      `}</style>
      <header className="md:hidden sticky top-0 z-20 bg-[#133e3b] text-stone-50 border-b-2 border-lime-400/70 shadow-sm">
        <div className="max-w-6xl mx-auto px-4 md:px-6 lg:px-8 py-3 flex items-center justify-between">
          ...
          <div className="flex gap-1">
            ...
            <label className="p-2 hover:bg-[#1a524d] rounded transition cursor-pointer" title="Import"><Upload className="w-4 h-4" /><input type="file" accept="application/json" onChange={importData} className="hidden" /></label>
            <button onClick={() => setShowSettings(true)} className="p-2 hover:bg-[#1a524d] rounded transition" title="Settings"><Settings className="w-4 h-4" /></button>
          </div>
        </div>
      </header>
      <main className="md:ml-60">
        <div className="max-w-6xl mx-auto px-4 md:px-6 lg:px-8 py-5 md:py-8 pb-24 md:pb-8">
      ...
      <nav className="md:hidden fixed bottom-0 inset-x-0 bg-white/95 backdrop-blur border-t border-stone-200/70 z-30 shadow-[0_-2px_10px_rgba(0,0,0,0.04)]">
      ...
      {/* SIDEBAR — desktop only (Phase B) */}
      <aside className="hidden md:flex fixed top-0 left-0 bottom-0 w-60 bg-[#133e3b] text-white flex-col z-40">
        <div className="p-5 flex items-center gap-3 border-b border-white/10">
          <div className="w-10 h-10 bg-lime-400 text-stone-900 rounded-md flex items-center justify-center font-display text-lg shadow-sm">P6</div>
          <div className="flex-1 min-w-0">
            <div className="font-display text-base leading-none">PEEM6PACK</div>
            <div className="text-[10px] text-white/50 leading-tight flex items-center gap-1 mt-1">
              v2.7 {isSyncing ? <CloudOff className="w-3 h-3 text-amber-300 animate-pulse" /> : <Cloud className="w-3 h-3 text-emerald-300" />}
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          {[
            ...
            { id: 'lock', icon: Lock, label: 'Lock List' },
            { id: 'log', icon: Database, label: 'Log & Dashboard' }
          ].map(item => {
            const Icon = item.icon;
            const active = page === item.id;
            return (<button key={item.id} onClick={() => setPage(item.id)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition text-sm ${active ? 'bg-lime-400 text-stone-900 font-bold shadow-sm' : 'text-white/80 hover:bg-white/10'}`}>
              <Icon className="w-4 h-4 flex-shrink-0" />
              <span>{item.label}</span>
            </button>);
          })}

          <button onClick={() => { setClipForVOnly(true); setShowAddClip(true); }} className="w-full mt-4 bg-lime-400 text-stone-900 font-bold flex items-center justify-center gap-2 py-2.5 rounded-lg hover:bg-lime-300 shadow-sm text-sm">
            <Plus className="w-4 h-4" /> + คลิปใหม่
          </button>
        </nav>

        <div className="p-3 border-t border-white/10 space-y-0.5">
          <button onClick={() => setShowBackup(true)} className="w-full flex items-center gap-3 px-3 py-2 text-xs text-white/80 hover:bg-white/10 rounded-md transition">
            <Download className="w-3.5 h-3.5" /> Backup / Restore
          </button>
          <label className="w-full flex items-center gap-3 px-3 py-2 text-xs text-white/80 hover:bg-white/10 rounded-md cursor-pointer transition">
            <Upload className="w-3.5 h-3.5" /> Import
            <input type="file" accept="application/json" onChange={importData} className="hidden" />
          </label>
          <button onClick={() => setShowSettings(true)} className="w-full flex items-center gap-3 px-3 py-2 text-xs text-white/80 hover:bg-white/10 rounded-md transition">
...
function StatCard({ icon: Icon, label, value, sub, trend, iconBg = 'bg-stone-100', iconColor = 'text-stone-600' }) {
  const trendColor = trend > 0 ? 'bg-emerald-50 text-emerald-700' : trend < 0 ? 'bg-rose-50 text-rose-700' : 'bg-stone-100 text-stone-500';
  return (<div className="bg-white border border-stone-200/70 rounded-xl p-3 md:p-4 shadow-sm relative">
    <div className="flex items-start justify-between mb-3">
      <div className={`${iconBg} rounded-lg p-2`}><Icon className={`w-4 h-4 ${iconColor}`} /></div>
      <div className="flex items-center gap-1">
        {typeof trend === 'number' && trend !== 0 && (<span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${trendColor}`}>{trend > 0 ? '↗' : '↘'} {Math.abs(trend)}%</span>)}
        <span className="text-stone-300 text-sm leading-none">⋮</span>
      </div>
    </div>
    <div className="text-[10px] text-stone-500 mb-1 font-semibold uppercase tracking-wide">{label}</div>
...
function FeaturedStatCard({ icon: Icon, label, value, sub, trend }) {
  return (<div className="bg-[#133e3b] text-white rounded-xl p-3 md:p-4 shadow-md relative">
    <div className="flex items-start justify-between mb-3">
      <div className="bg-lime-400 text-stone-900 rounded-lg p-2"><Icon className="w-4 h-4" /></div>
      <div className="flex items-center gap-1">
        {typeof trend === 'number' && trend !== 0 && (<span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${trend > 0 ? 'bg-emerald-500/30 text-emerald-200' : 'bg-rose-500/30 text-rose-200'}`}>{trend > 0 ? '↗' : '↘'} {Math.abs(trend)}%</span>)}
        <span className="text-white/40 text-sm leading-none">⋮</span>
      </div>
    </div>
    <div className="text-[10px] text-white/60 mb-1 font-semibold uppercase tracking-wide">{label}</div>
    <div className="font-display text-xl md:text-2xl leading-none">{value}</div>
...
function HomePage({ products, clips, lockedProducts, productsNeedingRescore, last7DaysClips, monthlyTarget, onSetMonthlyTarget, onGoTo, onSelectProduct, onEditClip, onMakeSimilar, onMarkRepostDone }) {
  ...
  const tiktokTotal7d = useMemo(() => products.reduce((s, p) => s + (Number(p.salesData?.last7d) || 0), 0), [products]);
  // Trend: 7d daily avg vs 30d daily avg  (last week vs last month)
  const tiktokTrend = useMemo(() => {
    const daily7d = tiktokTotal7d / 7;
    const daily30d = tiktokTotal30d / 30;
    if (daily30d === 0) return null;
    return Math.round(((daily7d / daily30d) - 1) * 100);
  }, [tiktokTotal7d, tiktokTotal30d]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        <StatCard icon={Package} label="สินค้า" value={products.length} sub={`Lock ${lockedProducts.length}`} iconBg="bg-stone-100" iconColor="text-stone-600" />
        <StatCard icon={Activity} label="คลิป 7 วัน" value={totalClips} sub={`${avgPerDay}/วัน`} iconBg="bg-sky-50" iconColor="text-sky-600" />
        <StatCard icon={DollarSign} label="GMV คลิป (เดือนนี้)" value={fmtNum(totalGMVMonth)} sub="จากที่บันทึก" iconBg="bg-violet-50" iconColor="text-violet-600" />
        <FeaturedStatCard icon={Flame} label="GMV จริง 30d" value={fmtNum(tiktokTotal30d)} sub={tiktokTotal30d > 0 ? "จาก TikTok manual" : "ยังไม่ได้กรอก"} trend={tiktokTrend} />
      </div>
